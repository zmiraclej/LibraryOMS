...
import com.flea.modules.ebook.util.ImageUti;
import com.flea.modules.ebook.util.SubString;
...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	public static boolean unZipFiles(String zipFileName, String descFileName) {
		...
		try {
			...
			while (enums.hasMoreElements()) {
				...
				// String(entry.getName().getBytes("GBK"),"utf-8");
				...
				if (entry.isDirectory()) {
					...
					// 如果entry是一个文件，则创建父目录
					File parent = new File(descFileDir).getParentFile();
					if (!parent.exists()) {
						parent.mkdirs();
					}
			...
			zipFile.close();
			log.debug("文件解压成功!");
			System.out.println("message:  文件解压成功!");
			return true;
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	...
	// urlPath.substring(urlPath.indexOf("/", 1), urlPath.length());
	// }
	// }
			...
			while (iter.hasNext()) {
				...
				if (file != null) {
					...
					if (myFileName.trim() != "") {
						...
						// 定义上传路径
						path = Config.getProperty("uploadFile") + folder + "/" + fileName;
						File localFileDir = new File(path.replace(fileName, ""));
						// 判断是否是文件夹 如果不是就创建文件夹
				...
				if (file != null) {
					...
					if (myFileName.trim() != "") {
						...
						if (!"ebooks".equals(folder)) {
							...
							String suffix = fileName.substring(fileName
									.lastIndexOf("."));
							fileName = prefix + "_" + str + suffix;
						}
						OSSUtils ossUtils = new OSSUtils();
						try {
							ossUtils.upFile(file.getInputStream(), folder + "/"
									+ parent + "/" + fileName);
						} catch (IOException e) {
							log.error(e.getMessage(), e);
							return "";
	...
	public static void uploadMutilpleFile(HttpServletRequest request,
			HttpServletResponse response, String folder) {
		String fileName, path = "";
		System.out.println(Config.getProperty("ebooksFile"));
		// 创建一个通用的多部分解析器
		CommonsMultipartResolver multipartResolver = new CommonsMultipartResolver(
				request.getSession().getServletContext());
		// 判断 request 是否有文件上传,即多部分请求
				...
				while (iter.hasNext()) {
					...
					if (file != null) {
						...
						if (myFileName.trim() != "") {
							...
							if (!"ebooks".equals(folder))
							// 通过时间格式加随机数来生成唯一的文件名
							fileName = str + splitFileNameToNumber(fileName);
							// 定义上传路径
							path = Config.getProperty("ebooksFile") + "/" + fileName;
	...
	public static boolean validateFileSize(HttpServletRequest request,
	...
	public static boolean validateImageSize(HttpServletRequest request,
			HttpServletResponse response) {
		String fileName, path = "";
		// 创建一个通用的多部分解析器
		CommonsMultipartResolver multipartResolver = new CommonsMultipartResolver(
	...
	// 修改上传上来的文件名，如果是中文的话
	private static String splitFileNameToNumber(String fileName) {
		...
		String str = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKMNOPQRSTUVWXYZ";
		char[] chars = new char[5];
		for (int i = 0; i < 5; i++) {
			char cha = str.charAt(rd.nextInt(40));
			chars[i] = cha;
		}
		int num1 = rd.nextInt(1000) + 100;
		return String.valueOf(chars) + num1 + "." + fileName;
	}
	 ...
	 * @throws IOException 
	 * 
	 * @Title: uploadFileImageAndEpub
	 * @Description: 电子书编辑功能的图片和epub文件上传
	 * @return String    返回类型
	 * @author QL 
	 * @date 2017年3月2日 上午10:31:05 
	 * @throws
	 */
	public static String uploadFileImageAndEpub(HttpServletRequest request,HttpServletResponse response,Ebook ebooks) throws IOException{
		 
		FileUtils.deleteDirectory(Config.getProperty("ebookFile"));
		//时间戳
		...
		if (multipartResolver.isMultipart(request)) {
			...
			while (iter.hasNext()) {
				...
				if (file != null) {
					// 取得当前上传文件的文件名称
					String myFileName = file.getOriginalFilename().trim();
					// 如果名称不为“”,说明该文件存在，否则说明该文件不存在
					if (StringUtils.isNotBlank(myFileName)) {
						// 扩展名jpg 和 epub
						fileName = FilenameUtils.getExtension(myFileName);
						//如果扩展名是jpg
						if ("jpg".equals(fileName) || "png".equals(fileName)) {
							// 定义上传路径
							path = Config.getProperty("ebookFile") + "pic/" + currentTimeMillis + "." + fileName;
							File localFileDir = new File(path);
							// 判断当前是否是文件夹, 如果不是就创建文件夹
						...
						//如果扩展名是epub
						if ("epub".equals(fileName)) {
							// 定义上传路径
							path = Config.getProperty("ebookFile") + "epub/" + FtpUtils.getSubCatalogByIsbn(currentTimeMillis + "") + currentTimeMillis + "." + fileName;
							...
							String epubName = path.substring(path.lastIndexOf("/") + 1);
							// 获取本地图片，上传到Ftp服务器 调用电子书epub文件上传方法,捕获异常
							FtpUtils ftpUtils = new FtpUtils();
							File epubDirFile = new File(path);
							try {
								// 重命名epub文件上传到ftp服务器
								ftpUtils.upload(epubDirFile, epubName, epubName);
								ebooks.setFile("ebooks/epub/" + FtpUtils.getSubCatalogByIsbn(currentTimeMillis + "") + currentTimeMillis + "." + fileName);
