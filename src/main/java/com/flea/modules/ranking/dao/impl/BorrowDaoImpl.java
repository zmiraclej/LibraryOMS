 ...
 */
@Repository
public class BorrowDaoImpl extends BaseDaoImpl<Clicklike,Integer> implements BorrowDao{
	/**
	    * 
	    * @method:find
	    ...
	    */
		...
		public Page<Clicklike> find(Page<Clicklike> page, String area, String lib) {
			...
			String sql = "SELECT t.* FROM ( SELECT CONVERT((@rowNum\\:=@rowNum+1.0),signed) AS ranking,a.* FROM (Select (@rowNum \\:=0) ) AS s,(SELECT lbk.isbn,lbk.properTitle AS bookName,lbk.press,lbk.price,lbk.author,DATE_FORMAT(lbb.borrowDate,'%Y')AS date,lbk.classificationNumber AS typeNumber,COUNT(lbk.isbn) AS click FROM librarys AS lb JOIN library_borrower_books AS lbb ON(lb.hallCode=lbb.hallCode) JOIN library_books AS lbk ON(lbb.libraryBookId=lbk.id) WHERE lb.isEffective = 1 and lb.customerId=? AND lb.areaAddress=?  AND lb.libraryLevel=?  GROUP BY lbk.isbn ORDER BY COUNT(lbk.isbn) DESC) AS a) t ";
	 ...
	 /**
	    ...
	    */
		...
		public Page<Clicklike> find(Page<Clicklike> page, QueryCriteria qc) {
			...
			sbf.append("SELECT t.* FROM ( SELECT CONVERT((@rowNum\\:=@rowNum+1.0),signed) AS ranking,a.*,a.click AS clickt FROM (Select (@rowNum \\:=0) ) AS s,(SELECT lbk.isbn,lbk.properTitle AS bookName,lbk.press,lbk.price,lbk.author,DATE_FORMAT(lbb.borrowDate,'%Y')AS date,lbk.classificationNumber AS typeNumber,COUNT(lbk.isbn) AS click FROM librarys AS lb JOIN library_borrower_books AS lbb ON(lb.hallCode=lbb.hallCode) JOIN library_books AS lbk ON(lbb.libraryBookId=lbk.id) WHERE lb.isEffective = 1 ");
		...
		private void getLibName(String option, StringBuffer sbf, QueryCriteria qc) {
			...
			case "9":
				...
				sbf.append(" GROUP BY lbk.isbn ORDER BY COUNT(lbk.isbn) DESC) AS a)t ");
				break;
			//名次 
			case "0":
				sbf.append(" GROUP BY lbk.isbn ORDER BY COUNT(lbk.isbn) DESC) AS a)t ");
			...
			case "8":
				...
				if(priceFrom.trim().length()>0 && priceTo.trim().length()>0){
					sbf.append(" HAVING COUNT(lbk.isbn) >= "+priceFrom.trim()+" AND COUNT(lbk.isbn) <= "+priceTo.trim()+" ");
				}else if(priceFrom.trim().length()>0 && priceTo.trim().length()==0){
					sbf.append(" HAVING COUNT(lbk.isbn) >= "+priceFrom.trim()+" ");
				}else if(priceFrom.trim().length()==0 && priceTo.trim().length()>0){
					sbf.append(" HAVING COUNT(lbk.isbn) <= "+priceTo.trim()+" ");
				}
				sbf.append(" ORDER BY COUNT(lbk.isbn) DESC) AS a)t ");
				break;
			}
		}
		/**
	    ...
	    */
		...
		public String getSum(String string, String string2) {
			...
			String str = "SELECT t.* FROM ( SELECT COUNT(a.isbn),SUM(a.click) FROM (Select (@rowNum \\:=0) ) AS s,(SELECT lbk.isbn,lbk.properTitle AS bookName,lbk.press,lbk.price,lbk.author,DATE_FORMAT(lbb.borrowDate,'%Y')AS date,lbk.classificationNumber AS typeNumber,COUNT(lbk.isbn) AS click FROM librarys AS lb JOIN library_borrower_books AS lbb ON(lb.hallCode=lbb.hallCode) JOIN library_books AS lbk ON(lbb.libraryBookId=lbk.id) WHERE lb.isEffective = 1 AND lb.customerId=? AND lb.areaAddress=?  AND lb.libraryLevel=?  GROUP BY lbk.isbn ORDER BY COUNT(lbk.isbn) DESC) AS a)t";
			if(Common.ROLE_FIRST_LEVLE.equals(le)){
